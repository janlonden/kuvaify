...
const links = {
  ...
  get () {
    const tempLinks = Array.from(document.querySelectorAll(options.linkElement + ' a'))
  ...
  addEventListeners () {
    links.links.forEach(link => {
      link.addEventListener('click', event => {
        ...
        const index = links.links.indexOf(link)
...
const img = {
  ...
  size () {
    const imgWidth = images[currentIndex].element.clientWidth
    const imgHeight = images[currentIndex].element.clientHeight
    let what
  ...
  visibility (what) {
    if (what === 'show') {
      ...
      if (options.transitionScale > 0) {
        img.transform({
          scaleValue: images[currentIndex].scale - options.transitionScale
        })
      }
  ...
  prepare (index) {
    ...
    if (!images[currentIndex].element.src) {
      ;(index => {
        ...
        images[thisIndex].element.addEventListener('load', () => {
  ...
  transform ({ scaleValue = images[currentIndex].scale, rotateValue = images[currentIndex].rotate }) {
  ...
  wheel (event) {
    ...
    img.transform({
      scaleValue: images[currentIndex].scale
    })
...
const rotate = {
  ...
  negativeRotate (event) {
    img.transform({
      rotateValue: images[currentIndex].rotate - 90
    })
